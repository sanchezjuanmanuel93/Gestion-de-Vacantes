<x-form-group fieldName="{{$fieldName}}" fieldDescription="{{$fieldDescription}}" :errors="$errors">
    <select name="{{$fieldName}}" id="{{$fieldId}}" class="form-control selectpicker show-tick" 
    @if ($searchOn) 
    ...
    @endif>
        ...
        <option value="{{$item[$keyField]}}"  @if($selected && $item[$keyField]==$selected) selected @endif>{{$item[$valueField]}}</option>
        @endforeach
    </select>
</x-form-group>